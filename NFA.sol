...
contract NFA is ERC721, ERC721Enumerable, Ownable {
    ...
    // config
    uint constant MAX_SUPPLY = 8051; // we come from area 51
    ...
    function reveal(string calldata _baseUri) public onlyOwner {
        require(bytes(baseURI).length == 0, "Metadatas can't be modified");
        baseURI = _baseUri;
    }
    ...
    function tokenURI(uint256 tokenId) public view override returns (string memory) {
        ...
        // return the unknown alien data for every NFT before reveal
        return collectionHasBeenRevealed ? string(abi.encodePacked(_baseURI(), tokenId.toString())) : "ipfs://QmYHAZK5FVuFqsp1kEprat22315c65iZXsZzY9SAcQALtp";
    }
